"""Wide chat-bots, relying on a single prompt and LLM to handle various user intents like in the zero shot agent example, can be difficult to manage.

But when tasks becomes more complex we may need specialized agents and workflows in owr Graph.
...
# USE CASE: We need an agentic graph that manages both flight booking and car rental booking.
"""This as an example of the supervisor design pattern from our Multi-agent examples.
...
flight_runnable = flight_booking_prompt | llm.bind_tools(
    [search_flights, update_ticket_to_new_flight, cancel_ticket] + [CompleteOrEscalate]
)
...
def search_car_rentals(
    ...
    """
    # Tool implementation would go here
    return [{"id": 1, "name": "Hertz", "price": 50}]  # Example result
...
def update_car_rental(
    ...
    """
    # Tool implementation would go here
    return "Car rental {rental_id} successfully updated."
...
car_rental_runnable = book_car_rental_prompt | llm.bind_tools(
    car_rental_tools + [CompleteOrEscalate]
)
...
def lookup_policy(query: str) -> str:
    ...
    return "Everything is in order. The user is allowed to make these changes."
...
"""We made the design decision to have a shared messages state between all the nodes. 
This is powerful in that each delegated assistant can see the entire user journey and have a shared context. This, however, means that weaker LLMs can easily get mixed up about there specific scope. 
To mark the "handoff" between the primary assistant and one of the delegated workflows we will add a ToolMessage to the state.
"""
