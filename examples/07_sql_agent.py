...
# Wrap the tool node with a fallback to handle errors
...
print("LIST TABLES RESULT: ", list_tables_tool.invoke(""))
print("GET SCHEMA RESULT: ", get_schema_tool.invoke("Artist"))
...
def db_query_tool(query: str) -> str:
    ...
    return result
print("QUERY EXECUTION RESULT: ", db_query_tool.invoke("SELECT * FROM Artist LIMIT 10;"))
...
query_llm_engine = ChatBedrock(
    model_id="anthropic.claude-3-haiku-20240307-v1:0",
    model_kwargs={"temperature": 0.5, "max_tokens": 2048},
).bind_tools([db_query_tool], tool_choice="db_query_tool")
query_check = query_check_prompt | query_llm_engine
print("QUERY CHECK RESULT: ", query_check.invoke({"messages": [("user", "SELECT * FROM Artist LIMIT 10;")]}))


# Create the Workflow for the agent
"""The workflow will consist of the following steps:
1. Fetch the available tables from the database with the list_tables_tool
2. Decide which tables are relevant to the question with the model_get_schema
3. Fetch the DDL for the relevant tables with the get_schema_tool
4. Generate a query based on the question and information from the DDL with the query_gen
5. Double-check the query for common mistakes using the model_check_query
"""
...
# Add a node for the first tool call (list_tables_tool)
...
graph_builder.add_edge("list_tables_tool", "model_get_schema")
graph_builder.add_edge("model_get_schema", "get_schema_tool")
...
while True: 
    ...
    # Example of a user input: Which sales agent made the most in sales in 2009?
    for event in graph.stream(
        {"messages": [("user", user_input)]}
    ):
        data = event.get("query_gen", {})
        if data.get("messages", None):
            if isinstance(data.get("messages")[-1], ToolMessage):
                print("Tool message", data.get("messages")[-1].content)
            elif isinstance(data.get("messages")[-1], AIMessage):
                print(data.get("messages")[-1].tool_calls[0]["args"]["final_answer"])
